...
app.post ('/profile-edit', (request, response)=> { 
    ...
    if (!firstname || !lastname || !email || !age) {
        return response.render('profile-edit', {
           error: 'Please complete your profile: Firstname, Lastname, Email, Age, City, Homepage',
    ...
    if(password) {
        passwordUpdatePr = hashing.generateHash(password)
        .then(passwordHash =>{
    ...
    const upsertPr = db.updateOrInsertUserProfile( request.session.userID, age, city, homepage);
